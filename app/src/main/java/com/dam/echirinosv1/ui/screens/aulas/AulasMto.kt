...
fun AulasMto(
    ...
    Column(modifier = modifier) {
        ...
        Button(
            onClick = {
                val aulaSelected = uiAulasBus.aulaSelected
                val ok = if (aulaSelected != -1) aulasVM.editar() else aulasVM.alta()
                if (ok) {
                    aulasVM.resetDatos()
                    onNavUp()
                    if (aulaSelected != -1) onShowSnackbar(contexto.getString(R.string.edicion_ok)) else onShowSnackbar(
                        contexto.getString(R.string.alta_ok)
                    )
                } else {
                    if (aulaSelected != -1) onShowSnackbar(contexto.getString(R.string.edicion_ko)) else onShowSnackbar(
