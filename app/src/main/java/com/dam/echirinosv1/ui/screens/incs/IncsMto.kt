...
import com.dam.echirinosv1.data.model.EstadoInc
...
fun IncsMto(
    ...
    Column(modifier = modifier.verticalScroll(rememberScrollState())) {
        ...
        ExposedDropdownMenuBox(
            ...
            TextField(
                value = incsVM.getNombreAula(uiIncMto.idAula),
                onValueChange = {},
                enabled = idDpto != 0,
                readOnly = true,
            ...
            ExposedDropdownMenu(
                ...
                if (idDpto != 0) {
                    ...
                    Datasource.aulas.filter { it.idDpto == idDpto }.forEach {
                        DropdownMenuItem(
        ...
        Button(
            onClick = {
                val incSelected = uiIncBus.incSelected
                val ok = if (incSelected != -1) incsVM.editar() else incsVM.alta()
                if (ok) {
                    incsVM.resetDatos()
                    onNavUp()
                    if (incSelected != -1) onShowSnackbar(contexto.getString(R.string.edicion_ok)) else onShowSnackbar(
                        contexto.getString(R.string.alta_ok)
                    )
                } else {
                    if (incSelected != -1) onShowSnackbar(contexto.getString(R.string.edicion_ko)) else onShowSnackbar(
